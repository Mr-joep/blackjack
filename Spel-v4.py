...
def calculate_hand_value(hand):
    ...
    while value > 21 and num_aces:
        value -= 10
        num_aces -= 1
     # Dit wordt gebruikt om de waarde van de azen aan te passen in de situatie dat de totaalwaarde boven de 21 ligt.
...
def blackjack():
    ...
    while True:
        ...
        # Stap 11: Blijf vragen of de speler opnieuw wil spelen
        while True:
            play_again = input("Wil je opnieuw spelen? (Ja/Nee): ").lower()
            if play_again in ['ja', 'nee']:
                break
            else:
                print("Ongeldige invoer. Kies 'Ja' of 'Nee'.")

        if play_again == 'nee':
            # Stap 12: Blijf vragen of de speler toch wil spelen
            while True:
                play_again = input("Wil je toch wel spelen? (Ja/Nee): ").lower()
                if play_again in ['ja', 'nee']:
                    break
                else:
                    print("Ongeldige invoer. Kies 'Ja' of 'Nee'.")
